...
import {
    ...
    CheckCircle, Clock3, XCircle, AlertCircle, UserPlus,
} from 'lucide-react';
...
const EventDetailPage: React.FC = () => {
        ...
        const loadOrganisation = () => {
            try {
                ...
                if (raw) {
                    ...
                    if (org?.id) setOrganisationId(org.id);
                }
            } catch (err) {
    ...
    const getRegistrationStatus = (ev: EventWithRegistration) => {
        if (!ev.registration_required || !ev.myReservation) return null;
        const { status } = ev.myReservation;
        if (status === 'confirmed') return { type: 'confirmed' as const, label: 'Inscrit', icon: CheckCircle };
        if (status === 'pending') return { type: 'pending' as const, label: 'En attente', icon: Clock3 };
        if (status === 'cancelled') return { type: 'cancelled' as const, label: 'Annulé', icon: XCircle };
        return null;
    };
    ...
    const canRegister = (ev: EventWithRegistration) => {
        ...
        if (ev.capacity && ev.available_spots !== null && (ev.available_spots ?? 1) <= 0) return false;
        return true;
    };
                ...
                {loading || !event ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <aside className="space-y-4">
                            ...
                            {/* Inscription */}
                            <div className="rounded-2xl bg-white/90 dark:bg-slate-900/90 backdrop-blur shadow-sm ring-1 ring-slate-200/70 dark:ring-slate-800/80 p-5 space-y-3">
